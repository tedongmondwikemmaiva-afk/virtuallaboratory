...
Public Class ChemicalsForm
    ...
    ' Reagent table rows, tracked separately since they live inside the table's
    ' RoundedPanel container rather than directly under `content` — the search
    ...
    Private Sub FilterReagents(query As String)
        ...
        For Each row In reagentRows
            Dim searchable As String = TryCast(row.Tag, String)
            row.Visible = (q = "") OrElse (searchable IsNot Nothing AndAlso searchable.Contains(q))
        Next
    End Sub
    ...
    Private Sub BuildReagentTable()
        ...
        For Each r In reagents
            Dim row = BuildReagentRow(panel, r.Item1, r.Item2, r.Item3, r.Item4, r.Item5, rowY, rowH, panelWidth,
    ...
    Private Function BuildReagentRow(panel As Panel, name As String, formula As String, conc As String,
                                 hazard As String, dotColor As Color, y As Integer, h As Integer, panelWidth As Integer,
                                 xReagent As Integer, xFormula As Integer, xConc As Integer, xHazard As Integer, xAction As Integer) As RoundedPanel
