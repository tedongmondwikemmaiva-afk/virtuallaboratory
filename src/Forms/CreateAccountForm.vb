...
Public Class CreateAccountForm
    ...
    Private Sub BuildUI()
        ' Borderless window: give it a small drag handle and a visible close button so it
        ' can still be moved and closed with the mouse, not just with the Escape key.
        Dim dragStrip As New Panel() With {.Size = New Size(Me.Width, 14), .Location = New Point(0, 0), .BackColor = Color.Transparent, .Cursor = Cursors.SizeAll}
        AddHandler dragStrip.MouseDown, Sub(s, e)
                                             ...
                                             End If
                                         End Sub
        card.Controls.Add(dragStrip)
        dragStrip.BringToFront()

        Dim btnClose As New Label() With {.Text = "✕", .Font = New Font("Segoe UI", 10), .ForeColor = Color.FromArgb(160, 168, 190),
                                           .Size = New Size(32, 28), .TextAlign = ContentAlignment.MiddleCenter, .Cursor = Cursors.Hand,
                                           .Location = New Point(Me.Width - 40, 8), .Anchor = AnchorStyles.Top Or AnchorStyles.Right, .BackColor = Color.Transparent}
    ...
    Private Async Sub BtnCreate_Click(sender As Object, e As EventArgs)
        ...
        Dim emailVal = ValueOrEmpty(txtEmail, "Institution email").Trim()
        ...
        Try
            ...
            Dim role As String = CStr(cmbRole.SelectedItem)
            Await UsersRepository.CreateAccountAsync(nameVal, emailVal, passVal, role)

            If role = "Teacher" Then
                ShowStatus($"Account created for {nameVal}. A Teacher account needs Admin approval before you can sign in.", True)
            Else
                ShowStatus($"Account created for {nameVal}. You can now sign in.", True)
            End If
            Me.DialogResult = DialogResult.OK
        Catch ex As Exception
            ShowStatus($"Couldn't reach the database: {ex.Message}", False)
        Finally
