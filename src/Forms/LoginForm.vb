...
Public Class LoginForm
    ...
    ' Borderless window: add a thin drag strip across the top and a visible close button
    ' so the window can be moved and closed with the mouse, not just with Escape.
    Private Sub BuildWindowChrome()
        Dim dragStrip As New Panel() With {.Dock = DockStyle.Top, .Height = 14, .BackColor = Color.Transparent, .Cursor = Cursors.SizeAll}
        ...
        Dim btnClose As New Label() With {.Text = "✕", .Font = New Font("Segoe UI", 10), .ForeColor = Color.FromArgb(160, 168, 190),
                                           .Size = New Size(40, 32), .TextAlign = ContentAlignment.MiddleCenter, .Cursor = Cursors.Hand,
                                           .Location = New Point(Me.ClientSize.Width - 40, 0), .Anchor = AnchorStyles.Top Or AnchorStyles.Right,
